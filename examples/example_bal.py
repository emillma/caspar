...
from caspar import CuFuncLib
from caspar import FactorLib
from caspar import Solver
...
if not (pt_path := fpath.with_suffix(".pt")).exists():
    ...
    def load(
        file: Path,
        typ: Type[np.number[Any]],
        start: int,
        len: int,
        cols: int | Sequence[int] | None = None,
    ) -> torch.Tensor:
...
def huber_norm(e: sf.Expr, k: sf.Expr) -> sf.Expr:
    if k == 0:
        return e
